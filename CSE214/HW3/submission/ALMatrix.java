...
public class ALMatrix {
	 ...
	 */
	/*
	 * O(nmk). n is numRows, m is numCols, and k is the capacity of mtx. 
	 * The presence of k is due to the amortized constant time of the add methods' grow().
	...
	public int get(int row, int col) {
		...
		return mtx.get(row*numRows+col);//row*numCols+col이게 맞음->여기서만 까임. 즉, 공식 틀려서 와장창 깎인거. 어떻게 막지?
	}
	 ...
	 */
	 ...
	 /*
	  * O(nmk). n is numRows, m is mtx's number of columns and k is the size of mtx.
	  * The presence of k is also related to the amortized constant time of the add methods' grow().
 	*/
	 ...
	 */
	 ...
	 /*
	  * O(nmk). n is mtx's number of rows, m is numCols, and k is the capacity of mtx.
	  * The presence of k is due to the amortized constant time of the add methods' grow().  
	  * The first loop has the highest order, so the first for loop decides the final big-O. 
	  */
	 ...
	 */
	 ...
	 /*
	  * O(nm). n is mtx's number of rows and m is mtx's number of columns.
	  */
